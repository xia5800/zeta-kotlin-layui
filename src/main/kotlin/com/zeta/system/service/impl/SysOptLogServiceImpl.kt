...
class SysOptLogServiceImpl: ISysOptLogService, ServiceImpl<SysOptLogMapper, SysOptLog>() {
     ...
     *
     * @param pageParam
     * @param queryParam PageParam<SysOptLogQueryParam>
     */
    override fun pageTable(pageParam: PageParam, queryParam: SysOptLogQueryParam?): PageResult<SysOptLogTableDTO> {
        val page = pageParam.buildPage<SysOptLogTableDTO>()
        // bug fix: 解决“分页查询参数model值为null时，mybatis的查询条件构造失败”问题 --by gcc
        val optLogList: List<SysOptLogTableDTO> = baseMapper.pageTable(page, queryParam ?: SysOptLogQueryParam())
        return PageResult(optLogList, page.total.toInt())
    }

...
class GlobalExceptionHandler {
    ...
    fun businessExceptionHandler(ex: BusinessException, request: HttpServletRequest): ApiResult<*> {
        logger.warn("抛出业务异常：${ex}")
        if (!isAjax(request)) {
            // 重定向到异常页面
            SaHolder.getResponse().redirect("/error/500")
        }
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    fun argumentExceptionHandler(ex: ArgumentException, request: HttpServletRequest): ApiResult<*> {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    fun illegalArgumentExceptionHandler(ex: IllegalArgumentException, request: HttpServletRequest): ApiResult<*> {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    fun limitExceptionHandler(ex: LimitException, request: HttpServletRequest): ApiResult<*> {
    ...
    fun notLoginExceptionHandler(ex: NotLoginException, request: HttpServletRequest): ApiResult<*> {
        logger.warn("抛出登录认证异常：", ex)
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    fun otherExceptionHandler(ex: Exception, request: HttpServletRequest): ApiResult<*> {
    ...
    private fun isAjax(request: HttpServletRequest): Boolean {
        val xHeader: String? = request.getHeader("X-Requested-With")
